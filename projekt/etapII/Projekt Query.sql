...
ALTER TABLE FACT_ACCIDENTS_TEMP
	ADD CONSTRAINT CONDITIONS_FOREIGN_KEY_TEMP FOREIGN KEY(Weather_Conditions_Id) REFERENCES DIM_CONDITIONS_TEMP(Id),
		CONSTRAINT ACCIDENT_FOREIGN_KEY_TEMP FOREIGN KEY(Flight_Id) REFERENCES DIM_FLIGHT_TEMP(Id),
		CONSTRAINT PLACE_FOREIGN_KEY_TEMP FOREIGN KEY(Place_Id) REFERENCES DIM_PLACE_TEMP(Id),
		CONSTRAINT PLANE_FOREIGN_KEY_TEMP FOREIGN KEY(Plane_Id) REFERENCES DIM_PLANE_TEMP(Id),
		CONSTRAINT EVENT_DATE_FOREIGN_KEY_TEMP FOREIGN KEY(Time_Id) REFERENCES DIM_TIME_TEMP(Id);

ALTER TABLE FACT_ACCIDENTS_TEMP
DROP CONSTRAINT CONDITIONS_FOREIGN_KEY_TEMP, ACCIDENT_FOREIGN_KEY_TEMP, PLACE_FOREIGN_KEY_TEMP, PLANE_FOREIGN_KEY_TEMP, EVENT_DATE_FOREIGN_KEY_TEMP;
...
INSERT INTO dbo.FACT_ACCIDENTS_TEMP(Flight_Id,Time_Id, Place_Id, Plane_Id, Weather_Conditions_Id, Total_Fatal_Injuries, Total_Serious_Injuries, Total_Minor_Injuries, Total_Uninjured, TotalOfInjured, Mortality)
SELECT
	DIM_FLIGHT_TEMP.Id AS Flight_Id,
	DIM_TIME_TEMP.Id AS Time_Id, 
	DIM_PLACE_TEMP.Id AS Place_Id, 
	DIM_PLANE_TEMP.Id AS Plane_Id,
	DIM_CONDITIONS_TEMP.Id AS Weather_Conditions_Id,
	...
	ISNULL(AviationData_TEMP.Total_Fatal_Injuries, 0) + ISNULL(AviationData_TEMP.Total_Serious_Injuries, 0) + ISNULL(AviationData_TEMP.Total_Minor_Injuries, 0) AS TotalOfInjured,
	CASE
		WHEN AviationData_TEMP.Total_Fatal_Injuries IS NULL THEN 0
		WHEN AviationData_TEMP.Total_Uninjured + AviationData_TEMP.Total_Serious_Injuries + AviationData_TEMP.Total_Minor_Injuries = 0 THEN 1
		ELSE AviationData_TEMP.Total_Fatal_Injuries * 1.0 / (AviationData_TEMP.Total_Uninjured + AviationData_TEMP.Total_Serious_Injuries + AviationData_TEMP.Total_Minor_Injuries + AviationData_TEMP.Total_Fatal_Injuries)
	END AS Mortality
FROM AviationData_TEMP 
JOIN DIM_FLIGHT_TEMP ON CONCAT(AviationData_TEMP.Investigation_Type,
AviationData_TEMP.Injury_Severity,AviationData_TEMP.Aircraft_Damage,
AviationData_TEMP.FAR_Description, AviationData_TEMP.Schedule,
AviationData_TEMP.Purpose_Of_Flight, AviationData_TEMP.Air_Carrier,
AviationData_TEMP.Broad_Phase_Of_Flight) = CONCAT(DIM_FLIGHT.Investigation_Type,
DIM_FLIGHT.Injury_Severity, DIM_FLIGHT.Aircraft_Damage,
DIM_FLIGHT.FAR_Description, DIM_FLIGHT.Schedule, DIM_FLIGHT.Purpose_Of_Flight,
DIM_FLIGHT.Air_Carrier, DIM_FLIGHT.Broad_Phase_Of_Flight)
JOIN DIM_TIME_TEMP ON DIM_TIME.Id = DATEPART(YYYY, Event_Date) * 10000 +
DATEPART(MM, Event_Date) * 100 + DATEPART(DD, Event_Date)
JOIN DIM_PLANE_TEMP ON CONCAT(DIM_PLANE.Make, DIM_PLANE.Model,
DIM_PLANE.Amateur_Built, CAST(DIM_PLANE.Number_Of_Engines AS nvarchar(2)),
DIM_PLANE.Engine_Type, DIM_PLANE.Aircraft_Category) = CONCAT(AviationData_TEMP.Make,
AviationData_TEMP.Model, AviationData_TEMP.Amateur_Built, CAST(AviationData_TEMP.Number_Of_Engines 
AS nvarchar(2)), AviationData_TEMP.Engine_Type, AviationData_TEMP.Aircraft_Category)
JOIN DIM_CONDITIONS_TEMP ON DIM_CONDITIONS.Weather_Condition =
AviationData_TEMP.Weather_Condition
JOIN DIM_PLACE_TEMP ON 
CONCAT(
	DIM_PLACE.Country,
	ISNULL(CONCAT_WS(', ', DIM_PLACE.District, DIM_PLACE.Region_Name), ''),
	DIM_PLACE.Latitude,
...
= 
CONCAT(
	AviationData_TEMP.Country,
	AviationData_TEMP.Location,
	AviationData_TEMP.Latitude,
	AviationData_TEMP.Longitude,
	AviationData_TEMP.Airport_Code,
	AviationData_TEMP.Airport_Name)
ORDER BY Flight_Id;
...
JOIN DIM_FLIGHT_TEMP ON CONCAT(AviationData_TEMP.Investigation_Type,
    ...
    AviationData_TEMP.FAR_Description, AviationData_TEMP.Schedule,
    AviationData_TEMP.Purpose_Of_Flight, AviationData_TEMP.Air_Carrier,
    AviationData_TEMP.Broad_Phase_Of_Flight) = CONCAT(DIM_FLIGHT_TEMP.Investigation_Type,
    DIM_FLIGHT_TEMP.Injury_Severity, DIM_FLIGHT_TEMP.Aircraft_Damage,
    DIM_FLIGHT_TEMP.FAR_Description, DIM_FLIGHT_TEMP.Schedule, DIM_FLIGHT_TEMP.Purpose_Of_Flight,
    DIM_FLIGHT_TEMP.Air_Carrier, DIM_FLIGHT_TEMP.Broad_Phase_Of_Flight)
JOIN DIM_TIME_TEMP ON DIM_TIME_TEMP.Id = DATEPART(YYYY, Event_Date) * 10000 +
    DATEPART(MM, Event_Date) * 100 + DATEPART(DD, Event_Date)
JOIN DIM_PLANE_TEMP ON CONCAT(DIM_PLANE_TEMP.Make, DIM_PLANE_TEMP.Model,
    DIM_PLANE_TEMP.Amateur_Built, CAST(DIM_PLANE_TEMP.Number_Of_Engines AS nvarchar(2)),
    DIM_PLANE_TEMP.Engine_Type, DIM_PLANE_TEMP.Aircraft_Category) = CONCAT(AviationData_TEMP.Make,
    AviationData_TEMP.Model, AviationData_TEMP.Amateur_Built, CAST(AviationData_TEMP.Number_Of_Engines 
    AS nvarchar(2)), AviationData_TEMP.Engine_Type, AviationData_TEMP.Aircraft_Category)
JOIN DIM_CONDITIONS_TEMP ON DIM_CONDITIONS_TEMP.Weather_Condition =
    AviationData_TEMP.Weather_Condition
JOIN DIM_PLACE_TEMP ON 
    CONCAT(
        DIM_PLACE_TEMP.Country,
        ISNULL(CONCAT_WS(', ', DIM_PLACE_TEMP.District, DIM_PLACE_TEMP.Region_Name), ''),
        DIM_PLACE_TEMP.Latitude,
        DIM_PLACE_TEMP.Longitude,
        DIM_PLACE_TEMP.Airport_Code,
        DIM_PLACE_TEMP.Airport_Name)
    = 
    CONCAT(
        AviationData_TEMP.Country,
        AviationData_TEMP.Location,
...
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[dbo].[CONDITIONS_FOREIGN_KEY_TEMP]') AND parent_object_id = OBJECT_ID(N'[dbo].[FACT_ACCIDENTS_TEMP]'))
ALTER TABLE FACT_ACCIDENTS_TEMP
    ADD CONSTRAINT CONDITIONS_FOREIGN_KEY_TEMP FOREIGN KEY(Weather_Conditions_Id) REFERENCES DIM_CONDITIONS_TEMP(Id);
...
SELECT COUNT(*) FROM AviationData_TEMP;

SELECT COUNT(*) FROM DIM_FLIGHT_TEMP;



DECLARE @D INT;
SET @D = (SELECT TOP 1 DATEPART(YYYY, Event_Date) * 10000 + DATEPART(MM, Event_Date) * 100 + DATEPART(DD, Event_Date) FROM AviationData_TEMP ORDER BY 1);

DECLARE @COUNTER DATE;
SET @COUNTER = CONVERT(date, CAST(@D AS nvarchar));

DECLARE @END INT;
SET @END = (SELECT TOP 1 DATEPART(YYYY, Event_Date) * 10000 + DATEPART(MM, Event_Date) * 100 + DATEPART(DD, Event_Date) FROM AviationData_TEMP ORDER BY 1 DESC);
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FACT_ACCIDENTS]') AND type in (N'U'))
BEGIN
    ALTER TABLE FACT_ACCIDENTS
        DROP CONSTRAINT CONDITIONS_FOREIGN_KEY, 
		ACCIDENT_FOREIGN_KEY,
        PLACE_FOREIGN_KEY,
        PLANE_FOREIGN_KEY,
        EVENT_DATE_FOREIGN_KEY;
END
...
SELECT * FROM FACT_ACCIDENTS_TEMP;

SELECT ETL_DATE, ETL_STATUS, ETL_MESSAGE FROM ETL_LOG;
